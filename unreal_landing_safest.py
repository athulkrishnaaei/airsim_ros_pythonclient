#Subcribe to pointcloud topic and apply ransac algorithm on to the point cloud and find the best place to land 
#it will first search for best place to land 
import setup_path
import airsim
import rclpy
...
class SafeLandingDetectionNode(Node):
    def __init__(self):
        ...
        self.accumulated_points = []  # List to store accumulated points
    ...
    def execute_mission(self):
        ...
        length = 5  # Length of the rectangular region
        width = 5  # Width of the rectangular region
        ...
        self.processing_executor.submit(self.process_accumulated_data)
    ...
    def pointcloud_callback(self, msg):
        ...
        try:
            ...
            self.logger.info(f'Accumulated points: {len(self.accumulated_points)}')
        except Exception as e:
            self.logger.error(f'Error converting PointCloud2 to numpy array: {e}')
            return

    def process_accumulated_data(self):
        # Combine all accumulated points into a single numpy array
        combined_points = np.vstack(self.accumulated_points)
        self.logger.info(f'Combined point cloud data shape: {combined_points.shape}')
        ...
        try:
            ...
            o3d.io.write_point_cloud("downsampled_point_cloud.pcd", o3d_cloud)
            self.logger.info('Saved the downsampled point cloud')
        except Exception as e:
    ...
    def is_safe_for_landing(self, cloud):
        ...
        max_height_variation = 100.0  # Maximum allowed height variation in the plane
...
def main(args=None):
    ...
    mission_thread = threading.Thread(target=node.execute_mission)
